...
class LudicrousDB extends wpdb {
	...
	public function add_database( array $db = array() ) {
		...
		$read = isset( $db['read'] ) ? (int) $db['read'] : 1;

		$write = isset( $db['write'] ) ? (int) $db['write'] : 1;
	 ...
	 *
	 * @param String $group Group.
	 * @param Array  $args Args, as array.
	 *
	 * @since 1.0.0
	 */
	...
	public function db_connect( $query = '' ) {
		...
		if ( isset( $this->ludicrous_tables[ $this->table ] ) ) {
			...
			if ( ! is_null( $this->callback_result ) ) {
				if ( is_array( $this->callback_result ) ) {
					extract( $this->callback_result, EXTR_OVERWRITE ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract
				} else {
		...
		do {
			...
			foreach ( $servers as $group_key ) {
				...
				// $group, $key
				extract( $group_key, EXTR_OVERWRITE ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract

				// $host, $user, $password, $name, $read, $write [, $lag_threshold, $timeout ].
				extract( $this->ludicrous_servers[ $dataset ][ $operation ][ $group ][ $key ], EXTR_OVERWRITE ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract
				$port = null;
				...
				if ( isset( $server ) && is_array( $server ) ) {
					extract( $server, EXTR_OVERWRITE ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract
				}
				...
				$lagged_status = $this->get_lag_cache();
				// Check for a lagged slave, if applicable.
				if ( empty( $use_master ) && empty( $write ) && ! isset( $ignore_slave_lag ) && isset( $this->lag_threshold ) && ! isset( $server['host'] ) && ( $lagged_status ) === DB_LAG_BEHIND ) {

					// If it is the last lagged slave and it is with the best preference we will ignore its lag.
					if ( ! isset( $unique_lagged_slaves[ $host_and_port ] ) && count( $unique_lagged_slaves ) + 1 === $unique_servers && $group === $min_group ) {
				...
				if ( $this->dbh_type_check( $this->dbhs[ $dbhname ] ) ) {
					 ...
					 */
					if ( empty( $use_master ) && empty( $write ) && ! isset( $ignore_slave_lag ) && isset( $this->lag_threshold ) && ! isset( $server['host'] ) && ( DB_LAG_OK !== $lagged_status ) && DB_LAG_BEHIND === ( $lagged_status ) && ! ( ! isset( $unique_lagged_slaves[ $host_and_port ] ) && ( ( count( $unique_lagged_slaves ) + 1 ) === $unique_servers ) && ( $group === $min_group ) ) ) {
						$unique_lagged_slaves[ $host_and_port ] = $this->lag;
						$this->disconnect( $dbhname );
						$this->dbhs[ $dbhname ] = false;
						$success                = false;
				...
				$msg .= "'tcp_responsive' => " . ( true === $tcp ? 'true' : $tcp ) . ",\n";
	 ...
	 * @since 1.0.0
	 *
	 * @param string $dbhname Database key.
	 * @param string $host Internet address: host:port of server on internet.
	 * @param string $user Database user.
	 * @param string $password Database password.
	 *
	 * @return bool|mysqli|resource false or database resource.
	 */
	 ...
	 * @since 1.0.0
	 *
	 * @param string $dbhname Database resource.
	 */
	...
	public function query( $query ) {
		...
		if ( preg_match( '/^\s*(create|alter|truncate|drop)\s/i', $query ) ) {
			...
			if ( ( true === $this->use_mysqli ) && ( $this->result instanceof mysqli_result ) ) {
				$this->load_col_info();
				while ( $row = mysqli_fetch_object( $this->result ) ) { // phpcs:ignore WordPress.CodeAnalysis.AssignmentInCondition.FoundInWhileCondition
					$this->last_result[ $num_rows ] = $row;
					$num_rows ++;
				}
			} elseif ( is_resource( $this->result ) ) {
				$this->load_col_info();
				while ( $row = mysql_fetch_object( $this->result ) ) {  // phpcs:ignore WordPress.CodeAnalysis.AssignmentInCondition.FoundInWhileCondition
					$this->last_result[ $num_rows ] = $row;
					$num_rows ++;
				}
			}
	 ...
	 * @param bool   $dbh_or_table Boolean, if query has run.
	 */
	 ...
	 * @param Float  $float_timeout Timeout time..
	 *
	...
	public function check_tcp_responsiveness( $host, $port, $float_timeout ) {
		...
		$socket = ( WP_DEBUG ) ? fsockopen( $host, $port, $errno, $errstr, $float_timeout ) : @fsockopen( $host, $port, $errno, $errstr, $float_timeout ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
